...
import { XAxis, Tooltip, YAxis, CartesianGrid, Legend, BarChart, Bar, LineChart, Line, AreaChart, Area } from 'recharts'
...
const EVgraph = ({ symbol }) => {
  ...
  const [yAxisFormat, setyAxisFormat] = useState(() => DataFormater)
  ...
  return (
    <div>
      ...
      {/* <LineChart width={500} height={250} data={EV} stackOffset={stackOffset} margin={{ top: 10, right: 30, left: 0, bottom: 0 }} > */}

      <LineChart width={500} height={250} data={EV} margin={{ top: 10, right: 30, left: 0, bottom: 0 }} >
        {/* <LineChart width={500} height={250} data={EV} margin={{ top: 10, right: 30, left: 0, bottom: 0 }} > */}
        ...
        <Legend />
        <Tooltip formatter={val => DataFormater(val)} />
        <Line type='monotone' name='Market Cap' dataKey='mkt_cap' stackId="1" stroke='#8884d8' fill='#8884d8' />
        <Line type='monotone' name='Net Debt' dataKey='net_debt' stackId="1" stroke='#82ca9d' fill='#82ca9d' />
      </LineChart>
    </div>
  )
}
